...
// Helper function to get hook addresses based on chain ID
function getHookAddresses(chainId: number) {
  if (chainId === baseSepolia.id) {
    return {
      staticHook: CLANKER_HOOK_STATIC_FEE_ADDRESS_SEPOLIA,
      dynamicHook: CLANKER_HOOK_DYNAMIC_FEE_ADDRESS_SEPOLIA,
    };
  } else if (chainId === base.id) {
    return {
      staticHook: CLANKER_HOOK_STATIC_FEE_V4,
      dynamicHook: CLANKER_HOOK_DYNAMIC_FEE_V4,
    };
  } else {
...
export function encodeFeeConfig(config: FeeConfig, chainId: number): {
  hook: Address;
  poolData: `0x${string}`;
} {
  const hooks = getHookAddresses(chainId);
  
  if (config.type === 'static') {
    return {
      hook: hooks.staticHook,
      poolData: encodeAbiParameters(STATIC_FEE_PARAMETERS, [config.clankerFee, config.pairedFee]),
    };
  } else {
    return {
      hook: hooks.dynamicHook,
