...
// Helper function to get contract addresses based on chain ID
function getContractAddresses(chainId: number) {
  if (chainId === baseSepolia.id) {
    return {
      factory: CLANKER_FACTORY_V4_SEPOLIA,
      locker: CLANKER_LOCKER_V4_SEPOLIA,
      vault: CLANKER_VAULT_ADDRESS_SEPOLIA,
      airdrop: CLANKER_AIRDROP_ADDRESS_SEPOLIA,
      devbuy: CLANKER_DEVBUY_ADDRESS_SEPOLIA,
      mevModule: CLANKER_MEV_MODULE_ADDRESS_SEPOLIA,
    };
...
export function buildTokenV4(
  ...
  const { hook, poolData } = encodeFeeConfig(feeConfig, chainId);
